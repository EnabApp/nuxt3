<template>
  <div p="4" h="80" grid="~ cols-4 gap-2" border="rounded-lg" overflow-y="scroll">
    ...
    <div @click="clicked(key)" v-for="(bussiness,key) in 4" :key="bussiness" cursor="pointer" grid="~ flow-row auto-rows-max" bg="w-10" place="items-center" p="4" border="rounded-lg 2" :class="key=1 ?'border-error-500' :'transition ease-in-out delay-75 hover:-translate-y-1 hover:scale-100 border-success-500'" >
